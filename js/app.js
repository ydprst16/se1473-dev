/*
| SE Monitoring Center — app.js v4  (Pencacah + Pengawas)
| - Global currentRole (default: pencacah)
| - Segmented pill tabs [data-role-tab]
| - Semua fetch history.php pakai ?role=...
| - KPI cards + progress adaptif per role
| - Backlog Approval color (khusus pengawas)
*/
...
const KPI_CONFIG = {
  ...
  pengawas: {
    ...
    cards: [
      ...
      { id: "rejected", title: "Rejected" },
      { id: "revoke", title: "Revoke" },
      { id: "progress", title: "Progress Approve" },
    ],
...
async function updateLastUpdate() {
  ...
  if (viewedDate) {
    try {
      const res = await fetch(apiURL("snapshot-meta", { date: viewedDate }), {
        cache: "no-store",
      });
      if (res.ok) {
...
async function init() {
  // PENTING: bindEvents duluan supaya tombol upload/tab tetap jalan
  // meskipun data awal gagal di-load (bucket kosong dll).
  bindEvents();
  applyRoleUI();
  ...
  try {
    ...
    // Bukan error fatal — tombol upload/tab tetap bisa dipakai
    Swal.fire({
      icon: "info",
...
async function switchRole(newRole) {
  ...
  try {
    ...
    toast(
      "info",
      `Menampilkan data ${newRole === "pengawas" ? "Pengawas" : "Pencacah"}`,
    );
  } catch (err) {
    hideLoading();
    // Kosongkan tampilan supaya jelas data belum ada
